...
- [~] ⭐ **product-aligned next build: on-device person-detection / counting** — camera → NPU count → emit one small anonymous event to atlas (NO image leaves the board). This is the wedge capability. **Software BUILT + VERIFIED 2026-06-25; only the physical flash + on-board verify remain (gated on a BOOT1 jumper move).**
  - **Model SETTLED (from compiled files, not docs):** NUCLEO flavor = **TinyYOLOv2 224×224** (`POSTPROCESS_OD_YOLO_V2_UF`, 5 anchors, SLA0044); DK flavor = YoloX-N 416. Repo cloned (our fork) at `../st-people` (sibling, not in git).
  - **Output interface SETTLED — the prebuilt firmware is OVERLAY-ONLY.** It computes count + boxes + **persistent track IDs** but renders them to pixels and discards them: UART1 carries status strings only, USB is UVC-video-only, no CDC. So OCR-the-overlay was rejected; instead we **forked the firmware and tapped the data at the source.**
  - **Firmware fork BUILT + SIGNED (flash-ready):** `Src/det_emit.c` + `Inc/det_emit.h` add `DetEmit_Frame()` — one `printf` line per inference frame over the existing USART1/VCP (no new UART init), called once in `app.c` right after the tracker finalizes `disp.info.tboxes[]`. Wire protocol (line-based, integer-only): `DET <seq> <ts_ms> <count> [<id>:<cx>:<cy>:<w>:<h>:<conf> ...]` (cx/cy/w/h permille, conf percent). `APP_DET_EMIT` toggle (default on). Provenance: `patches/st-people-uart-det-emit.patch` (+ `README-det-emit.md`). Builds with `scripts/build-people-detect.sh` (ARM GNU 13.3 + STM32_SigningTool); `DetEmit_Frame` confirmed linked, 79.5% AXISRAM. (Build needed one fix: stripped ST's `-fcyclomatic-complexity` flag — IAR-ism mainline GCC rejects.)
  - **Host bridge BUILT + TESTED:** `host/occupancy-bridge/` (`occbridge` package, stdlib-first, pyserial optional) — serial reader → protocol parser → debounce/heartbeat aggregator (with dwell scaffolding off the track IDs) → schema-v0 event → fail-open spine client. **44 unit tests green**; `python -m occbridge --replay tests/sample_capture.log --dry-run` emits the expected occupancy events (transient blips debounced). Replaces the never-built `pose-bridge.py` idea.
  - **REMAINING (physical — needs your hands):** (1) set **BOOT1 → position 2 (unprinted = dev-boot)**, power-cycle; (2) `scripts/flash-people-detect.sh` (programs FSBL + network_data + signed app); (3) set **BOOT0+BOOT1 → position 1 (printed)**, power-cycle to RUN; (4) on-board verify — board enumerates as UVC AND emits `DET …` lines on the newest `/dev/ttyACM*`; run `python -m occbridge --device /dev/ttyACM0 --dry-run`. **This run answers the real Step-1 questions on hardware:** does TinyYOLOv2 actually detect people here, and are the track IDs stable enough → decides count-only **v0** vs dwell/queue **v0.2**.
